...
current_date <- max(covid$date)
current_date <- covid %>%
  filter(date == current_date, cases > 0, county != "Unknown")
worst_counties <- current_date %>%
...
current_date <- max(covid$date)
current_date <- covid %>%
  filter(date == current_date, cases > 0)
state_death_case_ratio <- current_date %>%
  group_by(state) %>%
  summarize(total_cases=sum(cases, na.rm=TRUE), total_deaths=sum(deaths, na.rm=TRUE)) %>%
  ungroup()
state_death_case_ratio <- state_death_case_ratio %>%
  mutate(death_case_ratio = 100 * (total_deaths/total_cases)) %>%
  arrange(desc(death_case_ratio)) %>%
  slice(1:5)
state_death_case_ratio
```
